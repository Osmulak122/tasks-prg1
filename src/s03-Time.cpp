...
s24259::Time::Time(int hours, int minutes, int seconds)
        : hours{hours}, minutes{minutes}, seconds{seconds} {}

auto s24259::Time::next_hour() -> void {
    if (hours < 23) {
        ++hours;
    } else {
        hours = 0;
...
auto s24259::Time::next_minute() -> void {
    if (minutes < 59) {
        ++minutes;
    } else {
        minutes = 0;
...
auto s24259::Time::next_second() -> void {
    if (seconds < 59) {
        ++seconds;
    } else {
        seconds = 0;
...
auto s24259::Time::to_string() const -> std::string {
...
auto s24259::Time::to_string(Time_of_day daytime) const -> std::string {
    switch (daytime) {
        case morning:
            return "morning";
        case afternoon:
...
auto s24259::Time::time_of_day() const -> Time_of_day {
    if (hours >= 6 && hours <= 11) {
        return Time::morning;
...
auto s24259::Time::count_seconds() const -> std::uint64_t {
    auto s_count = hours * 3600 + minutes * 60 + seconds;
    return s_count;
}

auto s24259::Time::count_minutes() const -> std::uint64_t {
    auto m_count = hours * 60 + minutes;
    return m_count;
}

auto s24259::Time::time_to_midnight() const -> Time {
    Time t;
...
auto s24259::Time::operator+(const Time &n) const -> Time {
...
auto s24259::Time::operator!=(const Time &n) const -> bool {
...
auto main() -> int {
    auto time = s24259::Time{23, 59, 59};
    // to string
    ...
    auto a = s24259::Time{23, 59, 59};
    auto b = s24259::Time{0, 0, 1};
    std::cout << (a + b).to_string() << "\n\n";
    //  operator -
    std::cout << "output z operatora -" << std::endl;
    auto a1 = s24259::Time{3, 59, 59};
    auto b1 = s24259::Time{4, 0, 0};
    std::cout << (a1 - b1).to_string() << "\n\n";
    // operator <
    std::cout << "output z operatora <" << std::endl;
    auto a2 = s24259::Time{22, 59, 59};
    auto b2 = s24259::Time{23, 0, 1};
    if (a2 < b2) {
    ...
    // operator >
    std::cout << "output z operatora >" << std::endl;
    auto a3 = s24259::Time{23, 59, 59};
    auto b3 = s24259::Time{23, 0, 1};
    if (a3 > b3) {
    ...
    // operator ==
    std::cout << "output z operatora ==" << std::endl;
    auto a4 = s24259::Time{23, 59, 59};
    auto b4 = s24259::Time{0, 0, 1};
    if (a4 == b4) {
    ...
    // operator !=
    std::cout << "output z operatora !=" << std::endl;
    auto a5 = s24259::Time{23, 59, 59};
    auto b5 = s24259::Time{0, 0, 1};
    if (a5 != b5) {
